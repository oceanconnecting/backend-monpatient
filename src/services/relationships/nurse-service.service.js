...
export class NurseServiceService {
  static async createServiceRequest(patientId, requestData, reply) {
    try {
      ...
      if (
        error.code === "P2003" &&
        error.meta?.field_name?.includes("nurseId")
      ) {
  ...
  static async rateService(requestId, patientId, rating, feedback) {
    ...
    const averageRating =
      nurseRequests.reduce((acc, curr) => acc + curr.rating, 0) /
      nurseRequests.length;
  ...
  static async nursePatientsbyId(id) {
    const nurse = await prisma.patient.findMany({
      where: { id: id },
      include: {
        user: {
          select: {
